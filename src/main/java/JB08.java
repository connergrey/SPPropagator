 ...
 * </p>
 * <p>
 *  computation methods are proposed to the user:
 * <ul>
 ...
 */
public class JB08 implements Atmosphere {
    ...
    private JB08InputParams inputParams;
     ...
     * @see #JB08(JB08InputParams, PVCoordinatesProvider, BodyShape, TimeScale)
     */
    @DefaultDataContext
    public JB08(final JB08InputParams parameters,
...
/*        if (date.compareTo(inputParams.getMaxDate()) > 0 ||
        ...
        // calculate right ascension for the body (local siderial time)
        Frame eme2000 = FramesFactory.getEME2000();
        Transform frameToEme = frame.getTransformTo(eme2000, date);
        Vector3D posInEme = frameToEme.transformPosition(position);
        double bodyRA = MathUtils.normalizeAngle(posInEme.getAlpha(), FastMath.PI);


        // get sun position and calculate right ascension and declination
        Vector3D sunInEME = sun.getPVCoordinates(date, eme2000).getPosition();
        double sunRA = MathUtils.normalizeAngle(sunInEME.getAlpha(), FastMath.PI);
        double sunDec = sunInEME.getDelta();

        double[] solarDst = inputParams.getData(date);

        if (inBody.getAltitude() > ALT_MIN) {

            return getDensity(dateMJD, sunRA, sunDec,
                    bodyRA, inBody.getLatitude(), inBody.getAltitude(),
                    solarDst[0], solarDst[1],
                    solarDst[2], solarDst[3],
                    solarDst[4], solarDst[5],
                    solarDst[6], solarDst[7],
                    solarDst[8]);


        } else if (inBody.getAltitude() < ALT_MIN && inBody.getAltitude() > 0) {
            throw new OrekitException(OrekitMessages.ALTITUDE_BELOW_ALLOWED_THRESHOLD, inBody.getAltitude(), ALT_MIN);
            // Call the Msis model
            //System.out.println("LOW ALTITUDE WARNING");
            //eturn msis.getDensity(date,position,frame);
        } else {
            throw new OrekitException(OrekitMessages.ALTITUDE_BELOW_ALLOWED_THRESHOLD, inBody.getAltitude(), 0);
        }
    ...
    public <T extends CalculusFieldElement<T>> T getDensity(final FieldAbsoluteDate<T> date,
        ...
        double[] solarDst = inputParams.getData(dateD);
